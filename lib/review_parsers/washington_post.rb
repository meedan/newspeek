...
class WashingtonPost < ReviewParser
  ...
  def parse_raw_claim(raw_claim)
    pinocchios = raw_claim['page'].search('h3').map(&:text).select { |x| x.include?('Pinocchio') && !x.include?('Test') }[0]
    geppettos = raw_claim['page'].search('h3').map(&:text).select { |x| x.include?('Geppetto') }[0]
    ...
    if pinocchios.nil? && !geppettos.nil?
      ...
      score =
        begin
                       pinocchios.split(' ').map { |x| pinocchio_map[x] }.compact.first
