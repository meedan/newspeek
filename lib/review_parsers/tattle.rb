class Tattle < ReviewParser
  def get_claims
    ...
    raw_set.each_slice(100) do |claim_set|
      urls = claim_set.collect{|claim| claim["Post URL"] rescue nil}.compact
      existing_urls = ClaimReview.existing_urls(urls, self.class.service)
      new_claims = claim_set.select{|claim| !existing_urls.include?(claim["Post URL"])}
  ...
  def parse_raw_claim(raw_claim)
    {
      ...
      claim_body: (raw_claim["Docs"][0]["content"] rescue nil),
      claim_result: nil,
