class ClaimReview
  ...
  def self.save_claim(parsed_claim, service)
    ...
    f.close
    validated_claim = self.validate_claim(parsed_claim)
    self.repository.save(validated_claim.merge(service: service)) if validated_claim
  end

  def self.repository
    ClaimReviewRepository.new(client: self.client)
  end
  ...
  def self.existing_ids(ids, service)
    self.extract_matches(ids, "service_id", service)
  end
  ...
  def self.store_claim(parsed_claim, service)
    begin
      if self.existing_ids([parsed_claim[:service_id]], service).empty?
        self.save_claim(parsed_claim, service)
      end
    rescue
      binding.pry
    end
