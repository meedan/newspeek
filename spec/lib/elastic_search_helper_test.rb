...
describe ElasticSearchQuery do
  describe 'class' do
    ...
    it 'expects multi_match_against_service hash' do
      expect(ElasticSearchQuery.multi_match_against_service(%w[one two], 'ball', 'google')).to eq({ size: 2, from: 0, query: { bool: { must: [{ match_all: {} }], filter: [{ match_phrase: { 'service' => 'google' } }, { bool: { should: [{ match_phrase: { 'ball' => 'one' } }, { match_phrase: { 'ball' => 'two' } }], minimum_should_match: 1 } }], should: [], must_not: [] } } })
    end
